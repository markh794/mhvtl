...
#pragma ident	"@(#) $Id: vx_tape_bsd.c,v 1.3.2.1 2006-08-06 07:59:05 markh Exp $ vx_tape_bsd.c	1.1	05/06/27 SMI"
...
static int vx_tape_debug_blklist = 1;

/*
 * Some interesting statistics.  These are protected by the
 * vx_tape_stats_mutex.  It would be nice to have an ioctl to print them out,
 * but we don't have the development time for that now.  You can at least
 * look at them with adb.
 */

int		vx_tape_collect_stats = 1; /* Collect stats if non-zero */
kmutex_t	vx_tape_stats_mutex;	/* Protect these variables */
long		vx_tape_nowrite_count = 0; /* # active nowrite ranges */
static uint64_t	vx_tape_skipped_io = 0;	/* Skipped I/O operations, because of */
					/* VX_TAPE_WRITE_OFF. */
static uint64_t	vx_tape_skipped_blk = 0;	/* Skipped blocks because of */
					/* VX_TAPE_WRITE_OFF. */
static uint64_t	vx_tape_io_ops = 0;	/* Total number of I/O operations */
					/* including skipped and actual. */
static uint64_t	vx_tape_io_blocks = 0;	/* Total number of blocks involved */
					/* in I/O operations. */
static uint64_t	vx_tape_nonzero = 0;	/* Number of non-zero data blocks */
					/* currently held in memory */
static uint64_t	vx_tape_max_list_length = 0; /* Maximum size of a linked */
					    /* list of non-zero blocks. */
uint64_t vx_tape_taskq_max = 0;		/* vx_tape_scsi_start uses the taskq */
...
/*
 * Since vx_tape does no physical I/O, operations that would normally be I/O
 ...
 * machine is hung.
 *
 * To avoid this problem, we provide a function, vx_tape_yield_check, that does a
 * delay from time to time to yield up the CPU.  The following variables
 * are tunables for this algorithm.
 *
 *	vx_tape_num_delay_called	Number of times we called delay.  This is
 *				not really a tunable.  Rather it is a
 *				counter that provides useful information
 *				for adjusting the tunables.
 *	vx_tape_yield_length	Number of microseconds to yield the CPU.
 *	vx_tape_yield_period	Number of I/O operations between yields.
 *	vx_tape_yield_enable	vx_tape will yield the CPU, only if this
 *				variable contains a non-zero value.  This
 *				allows the yield functionality to be turned
 *				off for experimentation purposes.
 *
 * The value of 1000 for vx_tape_yield_period has been determined by
 * experience with running the tests.
 */
static uint64_t		vx_tape_num_delay_called = 0;
static int		vx_tape_yield_length = 1000;
static int		vx_tape_yield_period = 1000;
static int		vx_tape_yield_enable = 1;
static kmutex_t		vx_tape_yield_mutex;
static kcondvar_t 	vx_tape_yield_cv;

/*
 * This array establishes a set of tunable variables that can be set by
 * defining properties in the vx_tape.conf file.
 */
struct prop_map vx_tape_properties[] = {
	"vx_tape_collect_stats",		&vx_tape_collect_stats,
	"vx_tape_yield_length",		&vx_tape_yield_length,
	"vx_tape_yield_period",		&vx_tape_yield_period,
	"vx_tape_yield_enable",		&vx_tape_yield_enable,
	"vx_tape_max_task",		&vx_tape_max_task,
	"vx_tape_task_nthreads",		&vx_tape_task_nthreads
};

static unsigned char *vx_tape_zeros = NULL; /* Block of 0s for comparison */

extern void vx_tape_check_cond(struct scsi_pkt *pkt, uchar_t key,
...
static int bsd_readblks(struct vx_tape *, ushort_t, ushort_t, diskaddr_t,
				int, unsigned char *);
static int bsd_writeblks(struct vx_tape *, ushort_t, ushort_t, diskaddr_t,
				int, unsigned char *);
vx_tape_tgt_t *find_tgt(struct vx_tape *, ushort_t, ushort_t);
static blklist_t *bsd_findblk(vx_tape_tgt_t *, diskaddr_t, avl_index_t *);
static void bsd_allocblk(vx_tape_tgt_t *, diskaddr_t, caddr_t, avl_index_t);
static void bsd_freeblk(vx_tape_tgt_t *, blklist_t *);
static void vx_tape_yield_check();
static vx_tape_rng_overlap_t bsd_tgt_overlap(vx_tape_tgt_t *, diskaddr_t, int);

char *vx_tape_name = "vx_tape";
...
vx_tape_bsd_init()
{
	vx_tape_zeros = (unsigned char *) kmem_zalloc(DEV_BSIZE, KM_SLEEP);
	mutex_init(&vx_tape_stats_mutex, NULL, MUTEX_DRIVER, NULL);
	mutex_init(&vx_tape_yield_mutex, NULL, MUTEX_DRIVER, NULL);
	cv_init(&vx_tape_yield_cv, NULL, CV_DRIVER, NULL);
}
...
vx_tape_bsd_fini()
{
	cv_destroy(&vx_tape_yield_cv);
	mutex_destroy(&vx_tape_yield_mutex);
	mutex_destroy(&vx_tape_stats_mutex);
	if (vx_tape_zeros != NULL) {
		kmem_free(vx_tape_zeros, DEV_BSIZE);
		vx_tape_zeros = NULL;
	}
}
 ...
 * vx_tape_properties array.  If the property exists, copy its value to the
 ...
 */
void
vx_tape_bsd_get_props(dev_info_t *dip)
{
	uint_t		count;
	uint_t		i;
	struct prop_map	*pmp;
	...
	for (pmp = vx_tape_properties, i = 0;
		i < sizeof (vx_tape_properties) / sizeof (struct prop_map);
		i++, pmp++) {
...
int
vx_tape_bsd_blkcompare(const void *a1, const void *b1)
{
...
{
	cmn_err(CE_CONT, "bsd_scsi_start_stop()\n");
	return (0);
}
...
{
	cmn_err(CE_CONT, "bsd_scsi_request_sense()\n");
	return (0);
}
...
{
	struct vx_tape		*vx_tape = PKT2VX_TAPE(pkt);
	struct vx_tape_cmd	*sp = PKT2CMD(pkt);
	int			instance = ddi_get_instance(vx_tape->vx_tape_dip);
...
bsd_scsi_inquiry(struct scsi_pkt *pkt)
{
	...
	cmn_err(CE_CONT, "bsd_scsi_inquiry: ID: %d, LUN: %d\n",
			pkt->pkt_address.a_target,
			pkt->pkt_address.a_lun);

	if (sp->cmd_count < sizeof (inq)) {
		cmn_err(CE_CONT, "%s: bsd_scsi_inquiry: size %d required\n",
		    vx_tape_name, (int)sizeof (inq));
		return (EIO);
	}

	if (cdb->cdb_opaque[1] & 0xfc) {
		cmn_err(CE_WARN, "%s: bsd_scsi_inquiry: 0x%x",
		    vx_tape_name, cdb->cdb_opaque[1]);
		vx_tape_check_cond(pkt, 0x5, 0x24, 0x0);	/* inv. fld in cdb */
		return (0);
	}

	cmn_err(CE_CONT, "bsd_scsi_inquiry: dtype %d, inq type: 0x%x\n",
					tgt->vx_tape_tgt_dtype,
					cdb->cdb_opaque[2]);

	pqdtype = tgt->vx_tape_tgt_dtype;
	...
	(void) bcopy(tgt->vx_tape_tgt_inq, inq.inq_vid,
	    sizeof (tgt->vx_tape_tgt_inq));
...
bsd_scsi_io(struct scsi_pkt *pkt)
{
	...
	case SCMD_READ:
			...
			pkt->pkt_resid = bsd_readblks(sp->cmd_vx_tape,
					pkt->pkt_address.a_target,
					pkt->pkt_address.a_lun,
			...
			if (vx_tapedebug) {
				cmn_err(CE_CONT, "%s: bsd_scsi_io: "
				    ...
				    vx_tape_name, lblkno, lblkno, nblks);
			}
		break;
	case SCMD_WRITE:
			lblkno = (uint32_t)GETG0ADDR(cdb);
			nblks = GETG0COUNT(cdb);
			pkt->pkt_resid = bsd_writeblks(sp->cmd_vx_tape,
					pkt->pkt_address.a_target,
					pkt->pkt_address.a_lun,
					lblkno, nblks, sp->cmd_addr);
			if (vx_tapedebug) {
	...
	if (pkt->pkt_resid != 0)
		cmn_err(CE_WARN, "%s: bsd_scsi_io: "
		    ...
		    vx_tape_name, pkt->pkt_resid, lblkno, nblks);
	...
	case SCMD_MODE_SENSE:
			...
			if (vx_tapedebug) {
				cmn_err(CE_CONT, "%s: bsd_scsi_mode_sense: "
				    ...
				    vx_tape_name, page_code, page_control,
				    GETG0COUNT(cdb));
			}
		break;
...
bsd_mode_sense_dad_mode_geometry(struct scsi_pkt *pkt)
{
	...
	if (vx_tapedebug) {
		cmn_err(CE_CONT, "%s: bsd_mode_sense_dad_mode_geometry: "
		    "pc=%d n=%d\n", vx_tape_name, page_control, sp->cmd_count);
	}

	if (sp->cmd_count < (sizeof (header) + sizeof (page4))) {
		cmn_err(CE_CONT, "%s: bsd_mode_sense_dad_mode_geometry: "
		    "size %d required\n",
		    vx_tape_name, (int)(sizeof (header) + sizeof (page4)));
		return (EIO);
	}
...
bsd_mode_sense_dad_mode_err_recov(struct scsi_pkt *pkt)
{
	...
	if (sp->cmd_count < (sizeof (header) + sizeof (page1))) {
		cmn_err(CE_CONT, "%s: bsd_mode_sense_dad_mode_err_recov: "
		    "size %d required\n",
		    vx_tape_name, (int)(sizeof (header) + sizeof (page1)));
		return (EIO);
	}
...
bsd_mode_sense_modepage_disco_reco(struct scsi_pkt *pkt)
{
	...
	if (sp->cmd_count < (sizeof (header) + sizeof (page2))) {
		cmn_err(CE_CONT, "%s: bsd_mode_sense_modepage_disco_reco: "
		    "size %d required\n",
		    vx_tape_name, (int)(sizeof (header) + sizeof (page2)));
		return (EIO);
	}
...
bsd_mode_sense_dad_mode_format(struct scsi_pkt *pkt)
{
	...
	if (sp->cmd_count < (sizeof (header) + sizeof (page3))) {
		cmn_err(CE_CONT, "%s: bsd_mode_sense_dad_mode_format: "
		    "size %d required\n",
		    vx_tape_name, (int)(sizeof (header) + sizeof (page3)));
		return (EIO);
	}
	...
	case MODE_SENSE_PC_SAVED:
		...
		tgt = find_tgt(sp->cmd_vx_tape,
			pkt->pkt_address.a_target, pkt->pkt_address.a_lun);
		VX_TAPE_MUTEX_EXIT(sp->cmd_vx_tape);
		page3.sect_track = ushort_to_scsi_ushort(tgt->vx_tape_tgt_nsect);
		break;
	case MODE_SENSE_PC_CHANGEABLE:
		break;
	}
...
bsd_mode_sense_dad_mode_cache(struct scsi_pkt *pkt)
{
	...
	if (sp->cmd_count < (sizeof (header) + sizeof (page8))) {
		cmn_err(CE_CONT, "%s: bsd_mode_sense_dad_mode_cache: "
		    "size %d required\n",
		    vx_tape_name, (int)(sizeof (header) + sizeof (page8)));
		return (EIO);
	}
...
bsd_scsi_read_capacity_8(struct scsi_pkt *pkt)
{
	...
	tgt = find_tgt(sp->cmd_vx_tape,
		pkt->pkt_address.a_target, pkt->pkt_address.a_lun);
	VX_TAPE_MUTEX_EXIT(sp->cmd_vx_tape);
	if (tgt->vx_tape_tgt_sectors > 0xffffffff)
		cap.capacity = 0xffffffff;
	else
		cap.capacity =
		    uint32_to_scsi_uint32(tgt->vx_tape_tgt_sectors);
...
bsd_scsi_read_capacity_16(struct scsi_pkt *pkt)
{
	...
	cap.sc_capacity = uint64_to_scsi_uint64(tgt->vx_tape_tgt_sectors);
...
static int
bsd_readblks(struct vx_tape *vx_tape, ushort_t a_target, ushort_t a_lun,
		diskaddr_t blkno, int nblks, unsigned char *bufaddr)
{
	...
	if (vx_tapedebug) {
		cmn_err(CE_CONT, "%s: bsd_readblks: "
		    ...
		    vx_tape_name, a_target, a_lun, blkno, blkno, nblks);
	}

	vx_tape_yield_check();

	VX_TAPE_MUTEX_ENTER(vx_tape);
	tgt = find_tgt(vx_tape, a_target, a_lun);
	VX_TAPE_MUTEX_EXIT(vx_tape);
	if (tgt == NULL) {
		cmn_err(CE_WARN, "%s: bsd_readblks: no target for %d,%d\n",
		    vx_tape_name, a_target, a_lun);
		goto unlocked_out;
	}
	 ...
	 * of this I/O by grabbing vx_tape_tgt_nw_lock.  This will keep the
	 * results from our call to bsd_tgt_overlap from changing while we
	 * do the I/O.
	 */
	rw_enter(&tgt->vx_tape_tgt_nw_lock, RW_READER);
	...
	for (i = 0; i < nblks; i++) {
		if (vx_tape_debug_blklist)
			cmn_err(CE_CONT, "%s: bsd_readblks: "
			    "%d of %d: blkno %lld\n",
			    vx_tape_name, i+1, nblks, blkno);
		if (blkno > tgt->vx_tape_tgt_sectors)
			break;
...
static int
bsd_writeblks(struct vx_tape *vx_tape, ushort_t a_target, ushort_t a_lun,
		diskaddr_t blkno, int nblks, unsigned char *bufaddr)
{
	vx_tape_tgt_t	*tgt;
	blklist_t	*blk;
		...
		if ((overlap == O_NONE) ||
			...
			if (blkno > tgt->vx_tape_tgt_sectors) {
				cmn_err(CE_WARN, "%s: bsd_writeblks: "
				    ...
				    vx_tape_name, blkno,
				    tgt->vx_tape_tgt_sectors);
				break;
			}

			blk = bsd_findblk(tgt, blkno, &where);
			if (bcmp(bufaddr, vx_tape_zeros, DEV_BSIZE) == 0) {
				if (blk) {
					bsd_freeblk(tgt, blk);
				}
			} else {
...
vx_tape_tgt_t *
find_tgt(struct vx_tape *vx_tape, ushort_t a_target, ushort_t a_lun)
{
	vx_tape_tgt_t	*tgt;

	tgt = vx_tape->vx_tape_tgt;
	while (tgt) {
		if (tgt->vx_tape_tgt_saddr.a_target == a_target &&
		    tgt->vx_tape_tgt_saddr.a_lun == a_lun) {
			break;
		}
		tgt = tgt->vx_tape_tgt_next;
	}
	return (tgt);
 ...
 */
int
bsd_freeblkrange(vx_tape_tgt_t *tgt, vx_tape_range_t *range)
{
	blklist_t	*blk;
	blklist_t	*nextblk;

	ASSERT(mutex_owned(&tgt->vx_tape_tgt_blk_lock));
	for (blk = (blklist_t *)avl_first(&tgt->vx_tape_tgt_data);
		blk != NULL;
		blk = nextblk) {
		/*
		 ...
		 */
		nextblk = AVL_NEXT(&tgt->vx_tape_tgt_data, blk);

		if (vx_tape_overlap(range, blk->bl_blkno, (size_t)1) != O_NONE) {
			bsd_freeblk(tgt, blk);
		}
	}
	return (0);
...
static blklist_t *
bsd_findblk(vx_tape_tgt_t *tgt, diskaddr_t blkno, avl_index_t *where)
{
	blklist_t	*blk;
	blklist_t	search;
	...
	blk = (blklist_t *)avl_find(&tgt->vx_tape_tgt_data, &search, where);
	return (blk);
}


static void
bsd_allocblk(vx_tape_tgt_t *tgt,
		diskaddr_t blkno,
		caddr_t data,
		avl_index_t where)
{
	...
	avl_insert(&tgt->vx_tape_tgt_data, (void *) blk, where);

	if (vx_tape_collect_stats) {
		mutex_enter(&vx_tape_stats_mutex);
		vx_tape_nonzero++;
		tgt->vx_tape_list_length++;
		if (tgt->vx_tape_list_length > vx_tape_max_list_length) {
			vx_tape_max_list_length = tgt->vx_tape_list_length;
		}
		mutex_exit(&vx_tape_stats_mutex);
	}
}
...
static void
bsd_freeblk(vx_tape_tgt_t *tgt, blklist_t *blk)
{
	if (vx_tape_debug_blklist)
		cmn_err(CE_CONT, "%s: bsd_freeblk: <%d,%d> blk=%lld\n",
		    vx_tape_name, tgt->vx_tape_tgt_saddr.a_target,
		    tgt->vx_tape_tgt_saddr.a_lun, blk->bl_blkno);

	ASSERT(mutex_owned(&tgt->vx_tape_tgt_blk_lock));

	avl_remove(&tgt->vx_tape_tgt_data, (void *) blk);
	if (vx_tape_collect_stats) {
		mutex_enter(&vx_tape_stats_mutex);
		vx_tape_nonzero--;
		tgt->vx_tape_list_length--;
 ...
 *
 * NOTE:  Callers of this function must hold the tgt->vx_tape_tgt_nw_lock
 *	  lock.  For the purposes of this function, a reader lock is
 *	  sufficient.
 */
static vx_tape_rng_overlap_t
bsd_tgt_overlap(vx_tape_tgt_t *tgt, diskaddr_t blkno, int count)
{
	vx_tape_nowrite_t	*nw;
	vx_tape_rng_overlap_t	rv = O_NONE;

	for (nw = tgt->vx_tape_tgt_nowrite;
		(nw != NULL) && (rv == O_NONE);
		nw = nw->vx_tape_nwnext) {
		rv = vx_tape_overlap(&nw->vx_tape_blocked,
				    blkno,
				    (size_t)count);
	}
	return (rv);
 ...
 * in a CPU bound kernel when the device is an vx_tape device.  This makes
...
vx_tape_yield_check()
{
	static uint_t	vx_tape_io_count = 0;	/* # I/Os since last wait */
	static uint_t	vx_tape_waiting = FALSE;	/* TRUE -> a thread is in */
						/*   cv_timed wait. */
	clock_t		ticks;

	if (vx_tape_yield_enable == 0)
		return;

	mutex_enter(&vx_tape_yield_mutex);

	if (vx_tape_waiting == TRUE) {
		/*
		 ...
		 */
		cv_wait(&vx_tape_yield_cv, &vx_tape_yield_mutex);
	} else if (vx_tape_io_count++ > vx_tape_yield_period) {
		/*
		 * Set vx_tape_waiting to let other threads know that we
		 * have started the timer.
		 */
		vx_tape_waiting = TRUE;
		vx_tape_num_delay_called++;
		ticks = drv_usectohz(vx_tape_yield_length);
		if (ticks == 0)
			ticks = 1;
		(void) cv_timedwait(&vx_tape_yield_cv,
		    &vx_tape_yield_mutex, ddi_get_lbolt() + ticks);
		vx_tape_io_count = 0;
		vx_tape_waiting = FALSE;

		/* Broadcast in case others are waiting. */
		cv_broadcast(&vx_tape_yield_cv);
	}

	mutex_exit(&vx_tape_yield_mutex);
}
