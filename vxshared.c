...
extern u8 sense[];
void
mkSenseBuf(u8 sense_d, u32 sense_q, u8 *sense_flg) {
	u16 *sp;
...
void
resp_allow_prevent_removal(u8 * SCpnt, u8 * sense_flg) {
 ...
 */
s32
resp_read_position_long(loff_t pos, u8 * buf, u8 * sense_flg) {
//	loff_t pos = c_pos.blk_number;
	u64 partition = 1;
	u64 *lp;
...
s32
resp_read_position(loff_t pos, u8 * buf, u8 * sense_flg) {
	u64 *lp;
 ...
 */
s32
resp_report_lun(struct report_luns * rpLUNs, u8 *buf, u8 *sense_flg) {
 ...
 */
s32
resp_read_media_serial(u8 *sno, u8 *buf, u8 *sense_flg) {
	u64 size = 0L;
 ...
 */
struct mode *
find_pcode(u8 pcode, struct mode *m) {
	s32 a;
 ...
 */
struct mode *
alloc_mode_page(u8 pcode, struct mode *m, s32 size) {
	struct mode * mp;

	if((mp = find_pcode(pcode, m))) { // Find correct page
		mp->pcodePointer = malloc(size); // Alloc enough space for data
 ...
 */
s32
resp_mode_sense(u8 *cmd, u8 *buf, struct mode *m, u8 *sense_flg) {
	...
	*/
	...
	if(0x3f == pcode) {	// Return all pages
		for(a=1; a < 0x3f; a++) {	// Walk thru all possibilities
			if((smp = find_pcode(a, m)))
				len += add_pcode(smp, (u8 *)ap + len);
		}
	} else {
		if((smp = find_pcode(pcode, m)))
	...
	// Fill in header..
		...
		if(blockDescriptorLen) 	// If the length > 0, copy Block Desc.
 ...
 */
s32
resp_write_attribute(u8 *buf, u64 len, struct MAM *mam, u8 *sense_flg) {
	s32 a;
...
void
setTapeAlert(struct TapeAlert_page * ta, u64 flg) {
	...
	if(flg)
            if(verbose > 1)
                syslog(LOG_DAEMON|LOG_INFO," Setting TapeAlert flags 0x%llx",
								flg);
 ...
 */
void
getCommand(int cdev, struct vtl_header *vheadp, int count)
{
	int	status = 0;
	u8	* SCpnt;
...
int
chrdev_open(char *name, uint8_t minor) {
	FILE *f;
	char	devname[256];
	char	buf[256];
	int	devn;
